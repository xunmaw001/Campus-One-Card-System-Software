                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>余额</label>
                                        <input id="balance" value="0" name="balance" class="form-control" readonly
                                               placeholder="余额">
                                        <div id="div1">
                                            <button onclick="chongzhi()"  type="button" class="btn btn-success 充值">充值</button>
                                            <button onclick="jiaofei11()" type="button" class="btn btn-success 缴费">缴费</button>
                                        </div>
                                    </div>
                                        <input id="role" type="hidden" value="用户" name="role" class="form-control"
                                               placeholder="身份">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="col-sm-6">
                            <input id="chongzhi" placeholder="请输入要充值的金额" class="form-control">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">关闭
                        </button>
                        <button type="button" onclick="recharge()" class="btn btn-primary">
                            充值
                        </button>
                    </div>
                </div><!-- /.modal-content -->
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="col-sm-6">
                            <select id="jiaofei" class="form-control">
                            </select>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" onclick="jiaofei()">
                            缴费
                        </button>
                    </div>
                </div><!-- /.modal-content -->
...
<script>
    ...
    function recharge() {
        var mymessage = confirm("真的要充值 "+$("#chongzhi").val()+"￥ 吗？");
        if (mymessage == true) {
            http("yonghu/recharge?zhi="+$("#chongzhi").val(), "get", {}, (res) => {
                if(res.code == 0){
                alert('充值成功');
    ...
    function jiaofei() {
        var mymessage = confirm("真的要缴费吗？");
        if (mymessage == true) {
            console.log($("#jiaofei").val())
            http("yonghu/jiaofei?zhi="+$("#jiaofei").val(), "get", {}, (res) => {
                if(res.code == 0){
                alert('缴费成功');
                $("#balance").val(res.balance)
                $('#exampleModal').modal('hide')
    ...
    <!--  级联表的图片  -->
    function jiaofeileixingelect() {
        //填充下拉框选项
        http("jiaofeileixing/page?page=1&limit=100&sort=&order=&dicCode=sex_types", "GET", {}, (res) => {
            if(res.code == 0){
            var jiaofeileixingeOptions = res.data.list;
            var sexTypesSelectSearch = document.getElementById('jiaofei');
            if(sexTypesSelectSearch != null) {
                sexTypesSelectSearch.add(new Option('-请选择-',''));
                if (jiaofeileixingeOptions != null && jiaofeileixingeOptions.length > 0){
                    for (var i = 0; i < jiaofeileixingeOptions.length; i++) {
                        sexTypesSelectSearch.add(new Option(jiaofeileixingeOptions[i].name+"，金额为："+jiaofeileixingeOptions[i].money+"￥",jiaofeileixingeOptions[i].id));
                    }
                }
            }
        }
