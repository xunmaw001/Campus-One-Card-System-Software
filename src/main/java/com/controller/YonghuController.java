...
public class YonghuController {
    ...
    */
    ...
    public R recharge(Double zhi ,HttpServletRequest request) {
        Integer id = (Integer)request.getSession().getAttribute("userId");
        YonghuEntity yonghu = yonghuService.selectById(id);
        if(yonghu == null){
            return R.error();
        }
        yonghu.setBalance(yonghu.getBalance()+zhi);
        boolean b = yonghuService.updateById(yonghu);
        if(b){
            JiaofeijiluEntity jiaofeijilu = new JiaofeijiluEntity();
            jiaofeijilu.setYonghuId(id);
            jiaofeijilu.setCreateTime(new Date());
            jiaofeijilu.setXiaofeiTime(new Date());
            jiaofeijilu.setXfmoney(zhi);
            jiaofeijilu.setXfname("充值金额");
            jiaofeijiluService.insert(jiaofeijilu);
            return R.ok("充值成功").put("balance",yonghu.getBalance());
        }else{
            return R.error();
        }
    ...
    */
    ...
    public R jiaofei(Integer zhi ,HttpServletRequest request) {
        Integer id = (Integer)request.getSession().getAttribute("userId");
        JiaofeileixingEntity jiaofeileixing = jiaofeileixingService.selectById(zhi);
        YonghuEntity yonghu = yonghuService.selectById(id);
        if(yonghu == null){
            return R.error();
        }
        ...
        if(yonghu.getBalance()< jiaofeileixing.getMoney()){
            return R.error("余额不足请充值");
        }
        yonghu.setBalance(yonghu.getBalance()-jiaofeileixing.getMoney());
        boolean b = yonghuService.updateById(yonghu);
        if(b){
            JiaofeijiluEntity jiaofeijilu = new JiaofeijiluEntity();
            jiaofeijilu.setYonghuId(id);
            jiaofeijilu.setCreateTime(new Date());
            jiaofeijilu.setXiaofeiTime(new Date());
            jiaofeijilu.setXfmoney(jiaofeileixing.getMoney());
            jiaofeijilu.setXfname(jiaofeileixing.getName());
            jiaofeijiluService.insert(jiaofeijilu);
            return R.ok("缴费成功").put("balance",yonghu.getBalance());
        }else{
            return R.error();
        }
